...
DEST = {
    target: "target",
    prodZip: "ca-printable-forms.zip"
};
...
gulp.task(TASKS.compileFonts, function () {
    return gulp.src(APP.src.font)
        .pipe(gulp.dest(TEMP.embeddedFonts))
        .on('error', function (error) {
            console.log(error);
        })
});
...
gulp.task(TASKS.setUpTemp, gulpsync.sync([TASKS.cleanTemp, TASKS.webpack, TASKS.compileScss, TASKS.copyResourcesToTemp, TASKS.compileFonts]))
