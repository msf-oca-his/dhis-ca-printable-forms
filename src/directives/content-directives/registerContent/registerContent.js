TallySheets.directive('registerContent', [ 'Config', function(config) {
	return {
		...
		link: function($scope) {
			...
			$scope.rows = new Array(Math.floor((config.PageTypes.A4.LandScape.availableHeight - config.Register.headerHeight - config.Register.labelHeight) / config.Register.dataEntryRowHeight));
