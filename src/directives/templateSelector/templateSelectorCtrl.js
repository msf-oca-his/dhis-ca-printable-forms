TallySheets.directive('templateSelector', ['$rootScope', '$window', '$timeout', 'DataSetService', 'ProgramService', 'CustomAttributeService', 'Config', function ($rootScope, $window, $timeout, DataSetService, ProgramService, CustomAttributeService, config) {
    return {
        restrict: 'E',
        ...
        link: function ($scope, element) {
                    ...
                    .then(function (customAttributes) {
                        _.map(_.flatten(customAttributes), function (customAttribute) {
                            if (customAttribute.id == config.Attributes.printableUID) {
                                if (!customAttribute.dataSetAttribute) {
                                    alert("Custom attribute exists in the system but not assigned so please contact the system administrator.")
                                }
            ...
            var loadTemplates = function () {
                isAttributeExistsInSystem().then(function (isExists) {
                    if (!isExists && config.Attributes.printableUID != undefined) {
