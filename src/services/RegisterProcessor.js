TallySheets.service('RegisterProcessor', ['RegisterPage', 'Content', 'RegisterColumn', 'ContentTypes', 'Config', 'PrintFriendlyUtils', 'CustomAngularTranslateService', function(RegisterPage, Content, RegisterColumn, ContentTypes, config, PrintFriendlyUtils, CustomAngularTranslateService) {
	var page, currentPageIndex, pages, translatedCommentsValue;

	this.process = function(program, pageConfig) {
        return getTranslatedUseCode()
            .then(_.partial(processUtil, program, pageConfig));
	};
	...
	var processUtil = function (program, pageConfig, translatedCode) {
        translatedCommentsValue = translatedCode;
        ...
        registerColumnsPerPage.push(new RegisterColumn(translatedCommentsValue, defaultRenderType));
