TallySheets.service("DataElementService", ['$http', "OptionSetFactory", 'Config', function ($http, OptionSetFactory, config) {
    ...
    var getCustomAttributeForRenderingOptionSets = function (attributeValues) {
        return _.reduce(_.filter(attributeValues, function (attributeValue) {
            if (attributeValue.attribute.id === config.Attributes.printFlagUID) {
    ...
    var DataElement = function (data) {
        ...
        var printFlagAttribute = getCustomAttributeForRenderingOptionSets(data.attributeValues);
        if (printFlagAttribute) {
            dataElement.displayOption = printFlagAttribute.value;
        }
